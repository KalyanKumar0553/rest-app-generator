...
public class SubscriptionLookupService {
	...
	private static final Set<SubscriptionStatus> ACTIVE_STATUSES = EnumSet.of(
	...
	public SubscriptionPlanEntity getPlanByCode(String code) {
		return planRepository.findByCodeAndDeletedFalse(normalizeCode(code))
	...
	public SubscriptionPlanEntity getDefaultPlan() {
		return planRepository.findByIsDefaultTrueAndIsActiveTrueAndDeletedFalse()
				.orElseThrow(() -> new PlanNotFoundException("DEFAULT"));
	}
	...
	public SubscriptionFeatureEntity getFeatureByCode(String code) {
		return featureRepository.findByCodeAndDeletedFalse(normalizeCode(code))
	...
	public SubscriptionFeatureEntity getFeatureById(Long featureId) {
		return featureRepository.findById(featureId)
				...
				.orElseThrow(() -> new FeatureNotFoundException(String.valueOf(featureId)));
	}
	...
	public CustomerSubscriptionEntity findActiveSubscription(Long tenantId) {
		Optional<CustomerSubscriptionEntity> entity = customerSubscriptionRepository
				.findTopByTenantIdAndStatusInAndDeletedFalseOrderByCreatedAtDesc(tenantId, ACTIVE_STATUSES);
