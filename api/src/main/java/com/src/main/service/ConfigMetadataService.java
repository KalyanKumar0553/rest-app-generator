...
public class ConfigMetadataService {
	...
	public ConfigPropertyResponseDTO saveOrUpdateProperty(ConfigPropertySaveRequestDTO request) {
		ConfigProperty property = configPropertyRepository.findByPropertyKey(request.getPropertyKey()).map(p -> toEntity(request, p)).orElseGet(() -> toEntity(request, null));
	...
	public List<ConfigPropertyResponseDTO> getPropertiesByCategory(String category) {
		...
		return configPropertyRepository.findByCategoryOrderByPropertyKeyAsc(category.trim()).stream().map(this::toResponseDTO).collect(Collectors.toList());
	}
	...
	private LinkedHashSet<ConfigPropertyValue> toValueEntitySet(List<ConfigPropertyValueDTO> values) {
		return values == null ? new LinkedHashSet<>() :  // method reference you already have
		values.stream().filter(Objects::nonNull).map(this::toEntityValue).collect(Collectors.toCollection(LinkedHashSet::new));
	}
	...
	public ConfigMetadataService(final ConfigPropertyRepository configPropertyRepository, final DataSource dataSource) {
