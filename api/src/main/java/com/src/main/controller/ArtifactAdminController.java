...
public class ArtifactAdminController {
	...
	@PutMapping("/{appId}")
	@PreAuthorize("hasAuthority('artifact.app.manage')")
	public ResponseEntity<ArtifactAppResponseDTO> updateApp(
			@PathVariable UUID appId,
			@Valid @RequestBody ArtifactAppRequestDTO request,
			Principal principal) {
	...
	@PreAuthorize("hasAuthority('artifact.app.manage')")
	public ResponseEntity<ArtifactAppVersionResponseDTO> createVersion(
			@PathVariable UUID appId,
			@RequestBody(required = false) ArtifactVersionCreateRequestDTO request,
			Principal principal) {
		return ResponseEntity.ok(artifactAdminService.createVersion(
				appId,
				request == null ? null : request.getVersionCode(),
				currentUserId(principal)));
	}

	@PostMapping("/{appId}/publish")
	@PreAuthorize("hasAuthority('artifact.app.publish')")
	public ResponseEntity<ArtifactAppResponseDTO> publish(
			@PathVariable UUID appId,
			@RequestBody(required = false) ArtifactPublishRequestDTO request,
			Principal principal) {
		return ResponseEntity.ok(artifactAdminService.publish(
				appId,
				request == null ? null : request.getVersionCode(),
				currentUserId(principal)));
	}
