...
public class ProjectTabDefinitionAdminController {
	...
	public ResponseEntity<ProjectTabDefinitionAdminResponseDTO> create(
			@Valid @RequestBody ProjectTabDefinitionAdminRequestDTO request,
			Principal principal) {
		return ResponseEntity.status(HttpStatus.CREATED).body(projectTabDefinitionService.create(request, currentUserId(principal)));
	}
