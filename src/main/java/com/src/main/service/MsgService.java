...
public class MsgService {
	...
	private void sendEmail(String sender, String recipient, String subject,String html) {
		EmailMessage message = new EmailMessage().setSenderAddress(sender).setToRecipients(recipient)
				.setSubject(subject)
				.setBodyPlainText("")
				.setBodyHtml(html);
		SyncPoller<EmailSendResult, EmailSendResult> poller = getEmailClient().beginSend(message,null);
		PollResponse<EmailSendResult> response = poller.waitForCompletion();
		if(response.getValue().getStatus() == EmailSendStatus.FAILED || response.getValue().getStatus() == EmailSendStatus.CANCELED) {
			throw new SignupException(RequestStatus.EMAIL_SENT_ERROR);
		}
	}
	...
	public void sendSMS(String sender, String recipient,String msgTxt) {
		...
		Message message = Message.creator(
			      ...
			      new com.twilio.type.PhoneNumber(sender),msgTxt).create();
	}
}
