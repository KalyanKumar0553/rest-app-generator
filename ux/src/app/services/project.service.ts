...
export class ProjectService {
  ...
  getProjectTabDetails(generator?: string, dependencies: string[] = [], tabKey?: string): Observable<ProjectTabDefinition[]> {
    const queryParts: string[] = [];
    if (generator) {
      queryParts.push(`generator=${encodeURIComponent(generator)}`);
    }
    if (tabKey) {
      queryParts.push(`tabKey=${encodeURIComponent(tabKey)}`);
    }
    dependencies
