...
export class ProjectSpecMapperService {
  ...
  mapProjectToGeneratorSpec(project: any): any {
    const generatorLanguage = trimmed(project?.settings?.language) || 'java';
    const isNodeProject = generatorLanguage === 'node';
    const isPythonProject = generatorLanguage === 'python';
    const usesLightweightRuntime = isNodeProject || isPythonProject;
    ...
    const projectDescription = trimmed(project?.settings?.projectDescription) || 'Generated by Rest App Generator';
    const databaseCode = toDatabaseCode(project?.database?.database);
    const javaVersion = trimmed(project?.preferences?.javaVersion) || '17';
    const useJavaRecordsForDtos = this.supportsJavaRecords(javaVersion);
    ...
    const spec: any = {
      ...
      enableOpenapi: usesLightweightRuntime ? false : Boolean(project?.preferences?.enableOpenAPI),
      enableActuator: usesLightweightRuntime ? false : Boolean(project?.preferences?.enableActuator),
      enableLombok: usesLightweightRuntime ? false : Boolean(
      ...
      packages: usesLightweightRuntime ? 'technical' : (trimmed(project?.preferences?.packages) || 'technical'),
      ...
      dtos: this.mapDtos(project?.dataObjects, useJavaRecordsForDtos),
    ...
    if (isNodeProject) {
      spec.node = {
        packageManager: trimmed(project?.settings?.packageManager) || 'npm',
        port: Number(project?.settings?.serverPort) || 3000,
        orm: trimmed(project?.preferences?.orm) || 'prisma',
        docker: Boolean(project?.preferences?.useDockerCompose)
      };
      spec.orm = spec.node.orm;
    }

    if (isPythonProject) {
      spec.python = {
        orm: trimmed(project?.preferences?.orm) || 'sqlalchemy'
      };
      spec.orm = spec.python.orm;
    }

    if (!usesLightweightRuntime && Boolean(project?.preferences?.enableActuator)) {
  ...
  mapDtos(dataObjects: any, useJavaRecordsForDtos = false): any[] {
    ...
    return items.map((dataObject: any) => {
      ...
      if (!useJavaRecordsForDtos) {
        dto.classMethods = this.mapClassMethods(dataObject?.classMethods);
      }
  ...
  private supportsJavaRecords(javaVersion: any): boolean {
    const raw = trimmed(javaVersion);
    if (!raw) {
      return true;
    }
