...
@Component({
  ...
  styleUrls: ['./node-project-generation-dashboard.component.css']
})
export class NodeProjectGenerationDashboardComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('controllersRestConfigComponent') controllersRestConfigComponent?: NodeRestConfigComponent;
  ...
  readonly showPackageManager: boolean;
  ...
  constructor(
    ...
    this.runtimeLabel = this.runtimeLanguage === 'python' ? 'Python' : 'Node';
    this.apiFrameworkLabel = this.runtimeLanguage === 'python' ? 'FastAPI' : 'Express';
    this.showPackageManager = this.runtimeLanguage === 'node';
  }
  ...
  private resolveRuntimeLanguage(): 'node' | 'python' {
    const routeLanguage = String(this.route.snapshot.data['language'] ?? '').trim().toLowerCase();
    return routeLanguage === 'python' ? 'python' : 'node';
  }

  private normalizeRuntimeLanguage(value: string | null | undefined): 'node' | 'python' {
    const normalized = String(value ?? '').trim().toLowerCase();
    return normalized === 'python' ? 'python' : this.runtimeLanguage;
  }

  private defaultOrmForRuntime(): 'prisma' | 'sequelize' | 'sqlalchemy' | 'django' {
    return this.runtimeLanguage === 'python' ? 'sqlalchemy' : 'prisma';
  }
  ...
  private clearLegacyProjectStorageIfRequired(): void {
    const resetApplied = this.localStorageService.getItem(NodeProjectGenerationDashboardComponent.projectStorageResetKey);
    if (resetApplied === 'true') {
      return;
    }
    this.localStorageService.removeItem('projects');
    this.localStorageService.removeItem('project_zip_cache_v1');
    this.localStorageService.setItem(NodeProjectGenerationDashboardComponent.projectStorageResetKey, 'true');
  }
  ...
  private applyDraftTabData(section: string, response: ProjectDraftTabData): void {
    ...
    switch (section) {
      case 'general': {
        ...
        this.projectSettings.language = this.normalizeRuntimeLanguage(this.projectSettings.language);
        ...
        this.developerPreferences = {
          ...this.developerPreferences,
          ...preferences,
          orm: typeof (preferences as Record<string, unknown>)['orm'] === 'string'
            ? String((preferences as Record<string, unknown>)['orm'])
            : this.defaultOrmForRuntime(),
  ...
  private resetDraftBackedState(): void {
    ...
    this.projectSettings = { ...DEFAULT_PROJECT_SETTINGS, language: this.runtimeLanguage };
    this.databaseSettings = { ...DEFAULT_DATABASE_SETTINGS };
    this.developerPreferences = { ...DEFAULT_DEVELOPER_PREFERENCES, orm: this.defaultOrmForRuntime(), profiles: [] };
  }
  ...
  openDocumentation(
    event: Event,
    section: 'general' | 'entities' | 'dataObjects' | 'controllers' | 'mappers' | 'explore'
  ): void {
  ...
  filterDependencies(value: string): void {
    ...
    this.filteredDependencies = this.availableDependencies
      .filter(dep =>
        ...
        !NodeProjectGenerationDashboardComponent.shippableModuleKeys.includes(dep) &&
  ...
  private getSectionLoadedMessage(section: string): string {
    switch (section) {
      ...
      case 'cdn':
        return 'CDN configuration loaded';
      case 'controllers':
  ...
  private resumeProjectFromLocalDraft(savedProject: any, localProjectId: string): void {
    ...
    this.developerPreferences = {
      ...
      orm: typeof savedProject?.preferences?.orm === 'string' ? savedProject.preferences.orm : this.defaultOrmForRuntime(),
